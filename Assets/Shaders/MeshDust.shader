Shader "DustSucker/MeshDust" {
 Properties {
    ...
    CGPROGRAM
		...
		//textureless 3D simplex noise function from Ian McEwan, Ashima Arts
		...
		half snoise( half3 v ) {
			...
			m = m * m;
			return 50.0 * dot( m*m, half4( dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3) ) ); //42
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			// 1.5 - n, 1.0 - n, 0.5 - n 
			fixed4 c = half4( half3(n, n, n ), 1.0 ) * _Tint*n;
